...
- **Commits since Plan 3 (`<plan-3-final-sha>`):** <N> (16 spanning Tasks 1–15, 7 spanning Tasks 16–22)
- **Test count:** `pytest -q` reports **<N> passed** (Plan 3 baseline + ~80 new)
- **Lint + type-check:** `ruff check src/ tests/` clean; `mypy --strict src/mdpdf` clean.
- **All 16 acceptance criteria verified** with the commands in Task 22 §Step 2.
- **Next:** Plan 5 (UAT + Golden harness + Release pipeline).
