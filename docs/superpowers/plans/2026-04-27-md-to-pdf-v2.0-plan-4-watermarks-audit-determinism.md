...
- **Final commit hash:** updated post-completion to fix the two gaps below; see git log.
- **Commits since Plan 3 (`833bf89`):** 19 feature/test/doc commits + 1 follow-up patch (`fix(security): full bit-identical determinism + spec-compliant dc:creator`).
- **Test count:** `pytest -q` reports **385 passed, 5 skipped, 0 xfailed**. 5 environmentally gated (libcairo + KROKI_URL).
...
`pikepdf.Pdf.save(deterministic_id=True)` is now passed in both `apply_l2_xmp` and `freeze_pdf_dates`, deriving the trailer `/ID` from sha256 of the saved bytes instead of a random instance ID. End-to-end: same input + same options + same `--watermark-user` + same `SOURCE_DATE_EPOCH` produces a **byte-for-byte identical PDF** across runs. `dc:creator` is now written as a single-element list per the XMP standard, silencing pikepdf's deprecation warning.
