...
## Plan 5 — Completed (partial)

- **Date completed:** 2026-04-27
- **Final commit hash:** filled by the merge commit on PR.
- **Test count:** `pytest -q` reports **407 passed, 9 skipped, 1 warning** (Plan 4 baseline 385 → +22 new). Golden suite (`pytest tests/golden/`) reports **9 passed, 1 skipped** (libcairo gating branch_ops L3 locally). Lint + `mypy --strict` clean across 59 source files.

### Phases shipped

| Phase | Tasks | Status |
|---|---|---|
| 1 — UAT fixture + image assets | 1, 2 | ✅ committed |
| 2 — Golden harness foundation | 3, 4, 6 (L1 AST + L3 text-layer) | ⚠️ partial — L2 XMP + L4 layout-fp + L5 sha256 deferred to follow-up |
| 3 — Comprehensive integration + version bump | 9, 10 | ✅ committed (2.0.0a1 → 2.0.0) |
| 4 — CI matrix expansion (4 py × 3 OS + Kroki + libcairo) | 11 | ✅ committed |
| 5 — `doctor` + `fonts list/install` subcommands | 12, 13, 14 | ✅ committed |
| 6 — `--legacy-brand` deprecation | 15 | ✅ committed |
| 7 — Deletion (v1.8.9 monolith + brand_kits/ + legacy tests) | 16, 17, 18 | ⏭️ deferred — gated on L5 deterministic-sha256 baselines being green for every uat-*.md fixture (CLAUDE.md parity gate) |
| 8 — mkdocs scaffold + completion mark | 19 | ✅ committed |

### What's actually parity-gated

The CLAUDE.md gate text is "diff-zero output for every existing fixtures/uat-*.md plus branch_ops". This commit ships L1 (AST) + L3 (text-layer) golden snapshots that prove **v2.0 self-consistency** across runs but not v1↔v2 visual equivalence (the baselines are committed from v2.0 itself, not from v1.8.9). The strict literal interpretation of the gate would require a second snapshot layer that runs `scripts/md_to_pdf.py` (v1.8.9) on every uat fixture and diffs the rendered text — that work is deferred along with Phase 7 deletion.

### Follow-up PRs

1. **L5 deterministic sha256 baselines** — extends the golden harness to commit `sha256(deterministic.pdf)` per fixture, asserting the bit-identical contract from Plan 4 holds across all UAT fixtures.
2. **L2 XMP + L4 layout fingerprint** — completes the 5-layer harness from spec §7.2.
3. **Phase 7 — v1.8.9 monolith deletion** — `scripts/md_to_pdf.py` + helpers + `tests/test_md_to_pdf_*.py` + `brand_kits/`. Predicated on (1) shipping and going green.
4. **Sigstore-signed wheel publishing + GitHub Release pipeline** — Plan 5 §Task 21 (not authored as a sub-section in this draft; tracked in roadmap).
5. **mkdocs deployment** — `mkdocs gh-deploy` workflow lands once the docs content has had a polish pass.

### Next

v2.1 — template-pack system (`quote` / `user-manual` / `certificate`), per `docs/superpowers/specs/2026-04-25-md-to-pdf-v2.x-roadmap.md`.
